...
ระบบเป็น **standalone Apps Script project กลางที่เดียว** ไม่ผูกกับชีต (ไม่ container-bound) โดยชี้ไปยัง Google Sheet เป้าหมายด้วย **Sheet ID** ที่ตั้งค่าได้ ทำให้โค้ดชุดเดียวใช้กับชีตใดก็ได้ ไม่ต้องคัดลอกโค้ดไปวางในทุกชีต

หน้าตั้งค่าเปิดผ่าน **Web App** (deploy เป็น Web app แล้วเปิดผ่าน URL) ไม่ต้องใช้บริการภายนอกอื่น และไม่ต้องติดตั้ง library ใดๆ

---

## 1. Google Sheet Structure

ระบบเปิด Google Sheet เป้าหมายด้วย `SpreadsheetApp.openById(sheetId)` โดยอ่าน `sheetId` จาก Script Properties (ตั้งค่าจากหน้า Web App) ทั้งชีต `DATA` และ `Log` อยู่ในสเปรดชีตเดียวกันตาม Sheet ID นี้
...
ต้องมีหน้าตั้งค่า (Sidebar/HTML) ภาษาไทยทั้งหมด

เนื่องจากเป็น standalone project หน้าตั้งค่าเปิดผ่าน **Web App** — ใช้ `doGet()` เสิร์ฟไฟล์ `Sidebar.html` ผ่าน `HtmlService` แล้วเปิดจาก Web app URL (เมนู `onOpen` บนชีตใช้ไม่ได้ในโหมด standalone)

หน้าตั้งค่าต้องมีองค์ประกอบดังนี้:

0. ช่องกรอก `Sheet ID` ของ Google Sheet ที่มีชีต `DATA`
   - ต้องกรอกก่อนใช้งาน
   - ตอนบันทึกต้อง validate ว่าเปิด Sheet ID ได้จริง (`openById`) ถ้าไม่ได้ให้แจ้ง error ภาษาไทย
...
เนื้อหาอีเมลต้องส่งเป็น **HTML table** (`htmlBody`) เพื่อให้อ่านง่ายและคอลัมน์เรียงตรงในโปรแกรมอีเมล พร้อมแนบ **plain text** (`body`) เป็น fallback สำหรับ client ที่ไม่รองรับ HTML ต้อง escape อักขระพิเศษ (`< > & "`) ในค่าจากชีต และช่องที่ยาว เช่น `Media Location` ให้ตัดบรรทัดในเซลล์ (`word-break`)

ข้อมูลในอีเมล (ทั้ง HTML table และ plain text fallback) ต้องใช้ข้อมูลจากคอลัมน์:
...
ตัวอย่างรูปแบบตาราง plain text (ใช้เป็น fallback ควบคู่กับ HTML table):
...
- ไม่ต้องทำ action เพิ่มเติม และ**ไม่ต้อง retry** (ต่างจาก error ทั่วไปที่ retry 1 ครั้ง)
- ให้บันทึกลง `Log` เป็นสถานะ `QUOTA_EXCEEDED`
...
ให้ใช้ `PropertiesService.getScriptProperties()` สำหรับเก็บค่าตั้งค่า (standalone ไม่มี Document Properties จึงต้องใช้ Script Properties) เช่น:

- `Sheet ID` ของสเปรดชีตเป้าหมาย
...
- `doGet()`
  - Web App entry point — เสิร์ฟ `Sidebar.html` ผ่าน `HtmlService`
- `getSettingsForSidebar()`
- `saveSettingsFromSidebar(settings)`
- `toggleReminderSystemFromSidebar(isEnabled)`
...
- `getSpreadsheet()`
  - เปิดสเปรดชีตเป้าหมายด้วย `SpreadsheetApp.openById(sheetId)` จาก Sheet ID ใน Script Properties
  - ถ้า Sheet ID ว่างหรือเปิดไม่ได้ ให้ throw error ภาษาไทย
- `getDataRows()`
- `getColumnIndexMap(sheet)`
- `validateRequiredColumns(columnMap)`
- `getSpreadsheetUrl()`
...
- `sendTestEmail()` ใน `Code.gs` เป็นเพียง wrapper function สำหรับให้หน้า Web App (Sidebar) เรียกใช้งานผ่าน `google.script.run`
...
11. สร้าง email body แบบ HTML table (พร้อม plain text fallback)
